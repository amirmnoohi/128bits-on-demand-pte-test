/*
 * test2.c - Tests setting PTE meta with bit 58=0 (content)
 */
...
#define META_TYPE  0  // Set bit 58 to 0 (content)
...
int main(void)
{
    ...
    printf("    ✓ PTE meta set with bit 58=0\n");
    ...
    if (type != META_TYPE) {
        fprintf(stderr, "    ✗ expected type=0 (content), got type=%d\n", type);
        exit(EXIT_FAILURE);
    }
    if (meta != META_VALUE) {
